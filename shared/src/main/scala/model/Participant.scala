...
object Pant2Comp {
  ...
  def single(id: Long, coId: Long, status: PantStatus.Value) = new Pant2Comp(PantEntry.genSNO(id, id), coId, "", "", status, "_") 
  def double(id1: Long, id2: Long, coId: Long, status: PantStatus.Value) = new Pant2Comp(PantEntry.genSNO(id1, id2), coId, "", "", status, "_")
...
object PantEntry {
  implicit def rw: RW[PantEntry] = macroRW

  def decode(x: String): Either[Error, PantEntry] = {
    try Right(read[PantEntry](x)) 
    catch { case _: Throwable => Left(Error("err0054.decode.Pant2Comp", x.take(10), "", "Participant.decode")) }
  }

  def bye(name: String="bye") =  PantEntry(SNO.BYE, name, "", 0, (0, 0))
  def getBye(value: Int=0) =  PantEntry(SNO.bye(value).value, "", "", 0, (0, 0))
