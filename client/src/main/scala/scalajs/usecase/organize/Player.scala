  ...
  with TourneySvc with ViewServices
{
  ...
  override def update(param: String = "", upd: UpdateTrigger = UpdateTrigger("", 0L)) = {
    ...
    setHtml("MainCard", clientviews.organize.player.html.MainCard(player.filter(_._5 >= PLS_SICO))) 
    setHtml("WaitCard", clientviews.organize.player.html.WaitCard(player.filter(_._5 == PLS_WAIT))) 
    setHtml("RejectCard", clientviews.organize.player.html.RejectCard(player.filter(_._5 == PLS_RJEC)))
  
    // check if there are players with need signup commitment
    setBadge(player.filter(_._5 == PLS_SICO).length)
  }       

  def setBadge(cnt: Int = (-1)) = {
    val sicoCnt = if (cnt == -1 ) view4PlayerRegister.filter(_._5 == PLS_SICO).length else cnt    
    if (sicoCnt > 0) {
  ...
  def buttonSetStatus(coIdStr: String, sno: String, status: String): Unit = {
    ...
    status match {
      case "PLS_SIGN" => confirm(coId, sno, PLS_SIGN).map { if (_) setPlayerStatusUpdate(coId, sno, PLS_SIGN) }
      case "PLS_WAIT" => confirm(coId, sno, PLS_WAIT).map { if (_) setPlayerStatusUpdate(coId, sno, PLS_WAIT) }
      case "PLS_RJEC" => confirm(coId, sno, PLS_RJEC).map { if (_) setPlayerStatusUpdate(coId, sno, PLS_RJEC) }
      case "PLS_REDY" => setPlayerStatusUpdate(coId, sno, PLS_REDY)
      case "PLS_UNDO" => setPlayerStatusUpdate(coId, sno, PLS_SIGN)
  ...
  def confirm(coId: Long, sno: String, status: Int): Future[Boolean] = {
    ...
    val dlgMsg = status match {
      case PLS_RJEC => (getMsg("hdr.reject"),getMsg("body.reject"),getMsg("email.reject",email,firstname))
      case PLS_WAIT => (getMsg("hdr.wait"),getMsg("body.wait"),getMsg("email.wait",email,firstname))
      case PLS_SIGN => (getMsg("hdr.confirm"),getMsg("body.confirm"),getMsg("email.confirm",email,firstname))
